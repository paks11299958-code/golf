...
import { LogOut, Menu, X, ArrowLeft, Save } from 'lucide-react'
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(r => r.json()).then(u => {
      ...
      fetch(`/api/board/${id}`).then(r => r.json()).then(data => {
        if (data.error) { router.push('/board'); return }
        if (data.userId !== u.id) { router.push('/board'); return }
        setTitle(data.title)
    ...
    <main style={{ minHeight: '100vh', background: 'var(--ivory-50)', color: 'var(--ink-900)' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--r-xl)', padding: '28px 28px 24px' }}>
          <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--ink-900)', margin: '0 0 4px' }}>글 수정</h1>
          <p style={{ fontSize: 13, color: 'var(--fg-muted)', margin: '0 0 24px' }}>내용을 수정한 후 저장 버튼을 눌러주세요.</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={handleSubmit} disabled={submitting || success}
              ...
              <Save size={14} /> {submitting ? '저장 중...' : '수정 저장'}
            </button>
          </div>
        </div>
      </div>
